...
```
#### Linux → ya permitía ICMP por defecto
...
- El primer intento fallaba por timeout porque la clave privada no estaba en Windows Server.
...
ssh-keygen
```
#### 2.Copiar clave pública a Linux:
```bash
~/.ssh/authorized_keys
```
#### 3.Ajustar permisos:
